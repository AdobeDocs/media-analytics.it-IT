...
*In questa pagina viene illustrata la raccolta dati per la variabile **Originator**. Vedi [Iniziatore](/help/reporting/dimensions/originator.md) per la dimensione di reporting corrispondente.*

>[!ENDSHADEBOX]

La variabile del creatore è il creatore o lo studio di produzione del contenuto (ad esempio, `"Warner Brothers"`, `"Sony"` o `"Disney"`). Utilizzalo per confrontare il coinvolgimento tra proprietari di contenuti o titolari di diritti.

| Proprietà | Valore |
| --- | --- |
| **Variabile di dati di contesto** | `a.media.originator` |
| **Campo raccolta XDM** | [`xdm.mediaCollection.sessionDetails.originator`](https://experienceleague.adobe.com/en/docs/experience-platform/xdm/data-types/session-details-collection) |
...
Imposta `originator` all&#39;interno di `xdm.mediaCollection.sessionDetails` quando chiama [`sendEvent`](https://experienceleague.adobe.com/en/docs/experience-platform/collection/js/commands/sendevent/overview):
...
Utilizzare `MEDIA_VideoMetadataKeyORIGINATOR` per impostare l&#39;iniziatore nei metadati standard dell&#39;oggetto multimediale prima di chiamare `mediaTrackSessionStart`:
