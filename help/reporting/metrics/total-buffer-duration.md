...
*In questa pagina è inclusa la metrica **Durata totale buffer**. Adobe Analytics compila automaticamente una [durata totale del buffer (dimensione)](/help/reporting/dimensions/total-buffer-duration.md) associata dalla stessa variabile di dati di contesto `a.media.qoe.bufferTime`. Customer Journey Analytics espone un singolo campo `mediaReporting.qoeDataDetails.bufferTime` che è possibile utilizzare come dimensione o come metrica.*
...
Il backend del supporto somma la durata di ogni intervallo di buffer (da `media.bufferStart` alla successiva modifica dello stato). La metrica viene segnalata nella chiamata di chiusura. Analysis Workspace mostra il valore come `HH:MM:SS`; Feed dati, Data Warehouse e API di reporting mostrano il valore in secondi.
